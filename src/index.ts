...
function loop() {
  ...
  const brainThoughts: number[] = thinkBrain()
  game.ship.loadInputs(...brainThoughts)

  game.update()
  game.draw()

  if (game.ship.alive) window.requestAnimationFrame(loop)
  else console.log(`Brain has died and achieved a fitness of: ${brain.fitness}`)
}

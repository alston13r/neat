...
function neuralNetLoop() {
    ...
    for (let [i, pair] of desired.ordered.entries()) {
        neuralNetGraphics.createText('[' + pair.inputs.join(', ') + '] -> [' + neuralNet.feedForward(pair.inputs).join(', ') + ']', 5, 25 + i * 20, '#fff', 20, 'left', 'top').draw();
    }
    if (solutionFound)
        neuralNetGraphics.createText('solution', 5, 105, '#fff', 20, 'left', 'top').draw();
